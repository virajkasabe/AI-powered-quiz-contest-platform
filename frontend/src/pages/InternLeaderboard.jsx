           ...
           <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
             <div className="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-[14px]">
               <button
                 ...
                 className={`px-4 py-2 rounded-[10px] text-sm font-bold transition-all ${filterType === 'ALL' ? 'bg-white dark:bg-slate-700 text-sky-600 dark:text-sky-400 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
               >
                 All Interns
               </button>
               <button
                 onClick={() => setFilterType('DOMAIN')}
                 className={`px-4 py-2 rounded-[10px] text-sm font-bold transition-all ${filterType === 'DOMAIN' ? 'bg-white dark:bg-slate-700 text-sky-600 dark:text-sky-400 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
               >
                 My Domain
               </button>
             </div>
             ...
             filteredInterns.map((intern, i) => (
                ...
                >
                   ...
                   <div className="flex w-full items-center gap-6">
                      {/* Rank Ring */}
                      <div className={`shrink-0 w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center font-black text-xl shadow-md border 
                   ...
                   {/* Stats Block */}
                   <div className="w-full sm:w-auto flex flex-wrap sm:flex-nowrap items-center gap-6 md:gap-10 border-t sm:border-none border-slate-100 dark:border-slate-800 pt-4 sm:pt-0">
                      
                      <div className="flex flex-col gap-1 text-left sm:text-right w-[40%] sm:w-auto">
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Domain</span>
                        <span className="text-sm font-bold text-slate-700 dark:text-slate-300">{intern.domain}</span>
                      </div>

                      <div className="flex flex-col gap-1 text-left sm:text-right w-[40%] sm:w-auto sm:border-l sm:border-slate-200 dark:sm:border-slate-700 sm:pl-8">
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Total Score</span>
                        <span className="text-xl font-black text-emerald-500">{intern.score}</span>
                      </div>

                      <div className="flex flex-col gap-1 text-left sm:text-right w-full sm:w-auto sm:border-l sm:border-slate-200 dark:sm:border-slate-700 sm:pl-8 mt-2 sm:mt-0">
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Average</span>
                        <span className="text-xl font-black text-sky-500">{intern.percentage}%</span>
                      </div>
        ...
        {selectedIntern && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.3)] border border-slate-200 dark:border-slate-700 overflow-hidden"
            >
               <div className="h-24 bg-gradient-to-r from-sky-500 to-indigo-600 relative">
                  <button 
                   onClick={() => setSelectedIntern(null)}
                   className="absolute top-5 right-5 p-2 bg-white/20 hover:bg-white/30 rounded-xl text-white transition-colors"
                 >
               ...
               <div className="px-8 pb-8">
                  ...
                  <div className="flex justify-between items-end relative -top-10 mb-[-20px]">
                      <div className="w-20 h-20 bg-slate-100 dark:bg-slate-800 rounded-[1.2rem] border-[4px] border-white dark:border-slate-900 flex items-center justify-center shadow-lg font-black text-2xl text-slate-600 dark:text-slate-300">
                         {selectedIntern.name.charAt(0)}
                      </div>
                      <div className="pb-10 pl-4 w-full">
                  ...
                  <h2 className="text-3xl font-black text-slate-800 dark:text-white leading-tight">{selectedIntern.name}</h2>
                  <p className="text-slate-400 font-bold mb-6 text-sm">{selectedIntern.id}</p>

                  <div className="grid grid-cols-2 gap-4 mb-6">
                     <div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-2xl border border-slate-100 dark:border-slate-700 hover:border-sky-200 transition-colors">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Global Rank</p>
                        <p className="text-2xl font-black text-sky-500">#{selectedIntern.rank}</p>
                     </div>
                     <div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-2xl border border-slate-100 dark:border-slate-700 hover:border-sky-200 transition-colors">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Avg Accuracy</p>
                        <p className="text-2xl font-black text-indigo-500">{selectedIntern.percentage}%</p>
                     </div>
                     <div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-2xl border border-slate-100 dark:border-slate-700 hover:border-sky-200 transition-colors">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Score</p>
                        <p className="text-2xl font-black text-emerald-500">{selectedIntern.score}</p>
                     </div>
                     <div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-2xl border border-slate-100 dark:border-slate-700 hover:border-sky-200 transition-colors">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Attempts</p>
                        <p className="text-2xl font-black text-slate-700 dark:text-slate-200">{selectedIntern.totalQuizzes}</p>
                     </div>
                  </div>
